...
contract("TrustToken-Participation", accounts => {
  ...
  it("participate with transaction under goal", async () => {
    ...
    expect(participate.logs).to.have.lengthOf(1, "invalid number of events");
    ...
    const trusteeInfoTransaction = trustToken.isTrustee.call(firstTokenHolder, {
      from: firstTokenHolder
    });
    const transactions = [trusteeBalanceTransaction, trusteeInfoTransaction];
  ...
  it("participate with transaction equal to goal", async () => {
    ...
    // transaction events
    expect(participate.logs).to.have.lengthOf(3, "invalid number of events");
    ...
    // resolve transaction data
    const [trusteeTokenBalance, trusteeCount] = await Promise.all(transactions);
  ...
  it("participate with transaction over goal refunds excess", async () => {
    ...
    const refund =
      balanceBefore - balanceAfter - participate.receipt.gasUsed * 20000000000;
